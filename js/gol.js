...
class Cell {
  ...
  getNeighbors () {
    ...
    const [ n, s ] = [ i - cols, i + cols ]
    const [ e, w ] = [ i + 1, i - 1 ]
    const [ nw, ne ] = [ n - 1, n + 1 ]
    const [ sw, se ] = [ s - 1, s + 1 ]

    if ( col !== 0 ) { nbrs.push( w ); }
    if ( row !== 0 && col !== 0 ) { nbrs.push( nw ); }
    if ( row !== 0 ) { nbrs.push( n ); }
    if ( row !== 0 && col !== cols - 1 ) { nbrs.push( ne ); }
    if ( col !== cols - 1 ) { nbrs.push( e ); }
    if ( col !== cols - 1 && row !== rows ) { nbrs.push( se ); }
    if ( row !== rows ) { nbrs.push( s ); }
    if ( row !== rows && col !== 0 ) { nbrs.push( sw ); }
